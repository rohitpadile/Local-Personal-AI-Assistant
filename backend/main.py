...
def process_chat(req: ChatRequest):
    ...
    system_prompt = (
        ...
        f"CRITICAL USER CONTEXT (Recalled memories about the user):\n{memories_context}\n\n"
        "Guidelines:\n"
        "1. Respond directly and conversationally. Keep your responses relatively short and warm (typically 2-3 sentences max) for snappy voice replies, UNLESS the user explicitly requests a long explanation, teaching, lists, or detailed analysis.\n"
        "2. IMPORTANT: Use the 'CRITICAL USER CONTEXT' only when it is relevant to the user's current input. Do not divert the conversation to old memories if the user asks about an entirely new topic (like science, psychology, or general questions).\n"
        "3. If the user shares any personal facts, preferences, emotional states, or stories about themselves, you MUST note it for memory. "
        ...
        "4. If the user explicitly asks you to forget a fact, wipe out a memory, or corrects a detail you previously remembered, you MUST output what needs to be forgotten inside `<forget>...</forget>` tags at the very end of your response. "
